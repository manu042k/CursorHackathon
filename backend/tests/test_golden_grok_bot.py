"""US-X2: golden Grok Bot artifacts exist and match the paper contract / spec §10 shape."""
...
GOLDEN = ROOT / "data" / "experiments" / "grok-bot-seed-42"
FRONTEND_PAPER = ROOT / "frontend" / "src" / "data" / "grok-bot-seed-42.json"
...
def test_frontend_paper_validates_and_matches_spec_shape():
    ...
    assert paper.id == "grok-bot-seed-42"
    assert paper.experiment.product_name == "Grok Bot"
    assert paper.experiment.current_price == 120
    assert paper.experiment.competitor_price == 100
    ...
    assert paper.metrics.final_mrr_delta == 115
